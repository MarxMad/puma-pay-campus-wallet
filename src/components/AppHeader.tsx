    ...
    if (rightAction != null)
      return (
        <Button
          variant="ghost"
          size="icon"
          onClick={onRightAction ?? undefined}
          ...
          aria-label={onRightAction ? 'Acción' : undefined}
        >
          {rightAction}
          {showNotificationDot && (
            <span
              className="absolute top-1 right-1 w-2 h-2 bg-positive-500 rounded-full animate-pulse pointer-events-none"
              aria-hidden
            />
          )}
        </Button>
